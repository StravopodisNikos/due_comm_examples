...
void SPI0_Handler() {
  ...
  if ( (__waitSyncByte) && (!__breakTransfer) )
  {
    ...
    if (__newByte == (uint8_t) CMD_GET_STATE) {
      ...
      //SPI0->SPI_TDR = txbuf[0]; // sents the 
      Serial.println("4");
    }
      ...
      Serial.print("__newByte-3="); Serial.println(__newByte);
      __getModeTriggered = false;
      __setModeTriggered = true;
      __transferStillInProgress = true;
      __waitSyncByte = false;
      SPI0->SPI_TDR = (uint8_t) CMD_RECEIVED; //while (!SPI_SR_TDRE);
      Serial.println("5");
    }
    else {
  ...
  if ( (__transferStillInProgress) && (!__breakTransfer)  ){
    if (__setModeTriggered) { // Here the rest of the received bytes are only stored, nothing is sent back   
      Serial.println("7");
      cnt++;
      Serial.print("cnt="); Serial.println(cnt);
      //while (!SPI_SR_RDRF);
      __newByte = SPI0->SPI_RDR; // Read when data comes
      __newBytePtr = &__newByte;
      __slave_packet.bytes[cnt] = *__newBytePtr;
      SPI0->SPI_TDR = 0; //while (!SPI_SR_TDRE); // Write and wait data to be shifted out
      if (cnt == (SIZEOF_PACKET-1)) {
        __transferStillInProgress = false;
        __waitSyncByte = true;
        SPI0->SPI_RDR;
      }
    }
    else if (__getModeTriggered) { // Here, the received bytes are used only to send back the correct bytes
      Serial.println("8");
      Serial.print("cnt="); Serial.println(cnt);
      cnt++;       
    }
...
void loop() {
  ...
  if (__getModeTriggered && (!__transferStillInProgress)) {
    getModeSlave();
    __getModeTriggered = false;
    __waitSyncByte = true;    
  }
