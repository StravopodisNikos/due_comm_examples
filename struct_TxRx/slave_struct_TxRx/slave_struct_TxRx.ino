 ...
 * 
 * The MASTER (DUE1) sends a union data type message. The CMD_ID  
 * is of the category READ{POS/VEL/FORCE/CURRENT} and the value
 * is 0(not considered during request from master).
 * The SLAVE(DUE2) responds sending back the corresponding 
 * RESPONSE_ID {READ_SUCCESS, READ_FAILED} and the data (float) 
 * measurement.
 * For final operation only the master Serial Monitor should be 
 * checked.
 * This operation resembles the actual comm between master and slave
